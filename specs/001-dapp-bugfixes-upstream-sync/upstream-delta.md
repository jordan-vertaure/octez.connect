...
| SHA | Subject | Finding |
|-----|---------|---------|
| `2d9723ec6` | fix(dapp-client): unwedge in-flight requests on abort paths (+288/-105 in `DAppClient.ts`) | **NOT in the original 31-candidate enumeration (T022 gap).** Introduces `_initReject`, the `.finally` reject-observer chain, the defensive init-executor try/catch, the `makeRequest` try/finally `isInitPending` reset, and the single-channel `runRequestErrorSideEffects` refactor across all 8 request methods. Upstream's entire dApp-lifecycle chain (`32b83dcc0`+) treats `_initReject`/this refactor as **pre-existing context**. Our tree has `isInitPending` but **`_initReject` = 0** — our narrow `d82807184` port (the #33 fix) took only the `usesWrappedMessages()` guard, not the abort-path machinery. **This refactor must be re-expressed against our 5.0.0 tree before any dApp-lifecycle chain commit can apply.** It carries documented BEHAVIOR changes (DAppClient.init() can now reject; V3 `permissionRequest` throws raw `ErrorResponse`; `PERMISSION_REQUEST_ERROR` fires once with full payload) → release-gate extended e2e (Principle V) required. |
