...
describe('sendMessage unwrapping and routing', () => {
  ...
  it('routes a flat legacy (v2-dialect) permission request (WC peers have no beacon version)', async () => {
    const requestPermissions = jest
      .spyOn(client, 'requestPermissions')
      .mockResolvedValue(undefined)
    ...
    expect(requestPermissions).toHaveBeenCalledWith(
      expect.objectContaining({ network: { type: NetworkType.MAINNET }, scopes: ['sign'] })
    )
    expect((client as any).messageIds).toEqual(['flat-1'])
  })
