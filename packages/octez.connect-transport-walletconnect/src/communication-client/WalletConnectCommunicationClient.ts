...
/**
 * Map a flat legacy (v2-dialect) request onto the inner routing shape.
 * The flat type strings for operation/sign/broadcast/PoE requests are the
 * same wire strings the wrapped `blockchainData.type` uses, so a flat
 * message becomes a BlockchainRequest whose payload discriminator is its
 * own type; a flat permission request lifts its fields into blockchainData.
 */
const flatRequestToInner = (flat: Record<string, unknown>): InboundWrappedMessage => {
  if (flat.type === BeaconMessageType.PermissionRequest) {
    return {
      type: BeaconMessageType.PermissionRequest,
      blockchainData: {
        appMetadata: flat.appMetadata,
        scopes: flat.scopes,
        network: flat.network,
        networks: flat.networks
      }
    } as InboundPermissionRequest
  }

  return {
    type: BeaconMessageType.BlockchainRequest,
    blockchainData: { ...flat, type: flat.type }
  } as InboundBlockchainRequest
}
...
export class WalletConnectCommunicationClient extends CommunicationClient {
  ...
  async sendMessage(_message: string, _peer?: any): Promise<void> {
    ...
    if (typeof candidate.id !== 'string') {
      logger.warn('sendMessage', 'Dropping message without an id')

      return
    }

    // Negotiated wire: WC peers carry no beacon version, so the dApp serves
    // them the flat legacy dialect. Accept both shapes — a wrapped envelope
    // (generic API or future negotiation) is unwrapped; a flat message maps
    // its top-level fields onto the same inner-shape routing below.
    const inner: InboundWrappedMessage =
      unwrapBeaconMessage<InboundWrappedMessage>(candidate) ?? flatRequestToInner(candidate)

    // The dApp keys its openRequests on the envelope id (wrapper id for
    // wrapped requests, message id for flat ones).
