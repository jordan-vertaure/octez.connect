...
export class OutgoingResponseInterceptor {
  ...
  private static async handleFlatInput(config: OutgoingResponseInterceptorOptions): Promise<void> {
    ...
    const legacyDialect = !usesWrappedMessages(request.version)
    const requestInner = (request as BeaconMessageWrapper<BeaconBaseMessage>).message as
    ...
    switch (message.type) {
      case BeaconMessageType.Error: {
        ...
        if (legacyDialect) {
          ...
          interceptorCallback(flatResponse as unknown as BeaconMessage)
        } else {
          interceptorCallback(response as unknown as BeaconMessage)
        }
        break
  ...
  private static networkFromRequest(
    ...
    const data =
      (request as BeaconMessageWrapper<BeaconBaseMessage>).message !== undefined
        ? (
            (request as BeaconMessageWrapper<BeaconBaseMessage>).message as {
