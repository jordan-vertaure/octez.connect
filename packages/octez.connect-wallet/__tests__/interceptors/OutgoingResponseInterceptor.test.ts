...
describe('OutgoingResponseInterceptor — legacy flat v2 dialect (backward compatibility)', () => {
  const legacyRequest = {
    id: 'req-id',
    version: '2',
    senderId: 'dapp-sender',
  ...
  it('answers a flat v2 permission request with a FLAT response and still persists grants', async () => {
    const { config, interceptorCallback, addPermissions, validateResponse } = buildConfig({
      id: 'req-id',
      type: BeaconMessageType.PermissionResponse,
      network: { type: NetworkType.GHOSTNET },
      scopes: ['sign'],
    ...
    config.request = legacyRequest as any

    await OutgoingResponseInterceptor.intercept(config as any)

    const sent = interceptorCallback.mock.calls[0][0]
    // Flat dialect: no envelope, the legacy peer parses top-level fields.
    ...
    expect(sent).toMatchObject({
      ...
      senderId: 'wallet-sender',
      publicKey: 'edpkkey',
      appMetadata: { senderId: 'wallet-sender', name: 'Test Wallet' }
    })
    // The internal pipeline is shared: validation + ONE batched persistence.
    expect(validateResponse).toHaveBeenCalledTimes(1)
    expect(addPermissions).toHaveBeenCalledTimes(1)
  })

  it('answers a flat v2 operation request with a FLAT operation response', async () => {
    const { config, interceptorCallback } = buildConfig({
      id: 'req-id',
      type: BeaconMessageType.OperationResponse,
      transactionHash: 'oo123'
  ...
  it('answers a flat v2 request with a FLAT error (errorType at top level)', async () => {
    const { config, interceptorCallback } = buildConfig({
      id: 'req-id',
      type: BeaconMessageType.Error,
